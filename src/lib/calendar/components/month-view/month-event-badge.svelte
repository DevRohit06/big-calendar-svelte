<script lang="ts" module>
	...
	export const eventBadgeVariants = tv({
		base: 'mx-1 flex size-auto h-6.5 select-none items-center justify-between gap-1.5 truncate whitespace-nowrap rounded-md border px-2 text-xs focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring',
		variants: {
			color: {
	...
	type BadgeColor = VariantProps<typeof eventBadgeVariants>['color'];
	type Position = 'first' | 'middle' | 'last' | 'none';
</script>
...
<script lang="ts">
	...
	const itemStart = $derived(startOfDay(parseISO(event.startDate)));
	const itemEnd = $derived(endOfDay(parseISO(event.endDate)));

	const outsideRange = $derived(cellDate < itemStart || cellDate > itemEnd);
	...
	const renderBadgeText = $derived(['first', 'none'].includes(position));

	const color = $derived(
		(calendar.badgeVariant === 'dot' ? `${event.color}-dot` : event.color) as BadgeColor
	);
</script>
		...
		<EventDetailsDialog {event}>
			{#snippet children(triggerProps)}
				<div
					...
					role="button"
					tabindex={0}
					class={eventBadgeVariants({ color, multiDayPosition: position, class: className })}
				>
					<div class="flex items-center gap-1.5 truncate">
						{#if !['middle', 'last'].includes(position) && ['mixed', 'dot'].includes(calendar.badgeVariant)}
						...
						{#if renderBadgeText}
							<p class="flex-1 truncate font-semibold">
								{#if eventCurrentDay}
									<span class="text-xs">Day {eventCurrentDay} of {eventTotalDays} • </span>
								{/if}
								{event.title}
							</p>
						{/if}
